...
namespace pso
{
    ...
    private:
            ...
            Vector &fvals)
        {
            ...
            for(Index i = 0; i < particles.cols(); ++i)
                fvals(i) = objective_(particles.col(i),i);
        }
            ...
            Matrix &particles)
        {
            ...
            // HTP - I added this "callback" to tell the particles to update pos in the simulation and for the physics to update
            // iterations = -1 will make it perform this function
            bool callbackResult = callback_(-1, particles, bestFvals, gbest);
                ...
                fchange > feps_ && xchange > xeps_)
            {
                ...
                // HTP - I added this "callback" to tell the particles to update pos in the simulation and for the physics to update
                // iterations = -1 will make it perform this function
                callbackResult = callback_(-1, particles,
                    bestFvals, gbest);
                ...
                callbackResult = callback_(iterations, bestParticles,
                    bestFvals, gbest);
    ...
    public:
        ...
        ParticleSwarmOptimization(Objective objective, Callback callback)
            : objective_(objective), callback_(callback), weightStrategy_(), threads_(1),
            maxIt_(0), xeps_(static_cast<Scalar>(1e-6)),
            feps_(static_cast<Scalar>(1e-6)), phip_(static_cast<Scalar>(2.0)),
            phig_(static_cast<Scalar>(2.0)), maxVel_(static_cast<Scalar>(0.0)),
            verbosity_(0), dice_()
